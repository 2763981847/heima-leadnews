...
public class CacheService extends CachingConfigurerSupport {
    ...
    /**
     * 将给定 key 的值设为 JSON.toJSONString(value) ，并返回 key 的旧值(old JSON.toJSONString(value))
     *
     * @param key
     * @param value
     * @return
    ...
    public String getAndSet(String key, Object value) {
        return stringRedisTemplate.opsForValue().getAndSet(key, JSON.toJSONString(value));
    }
    ...
    /**
     * 将值 JSON.toJSONString(value) 关联到 key ，并将 key 的过期时间设为 timeout
     *
     * @param key
     * @param value
     * @param timeout 过期时间
    ...
    /**
     * 用 JSON.toJSONString(value) 参数覆写给定 key 所储存的字符串值，从偏移量 offset 开始
     *
     * @param key
     * @param value
    ...
    public void setRange(String key, Object value, long offset) {
        stringRedisTemplate.opsForValue().set(key, JSON.toJSONString(value), offset);
    }
    ...
    /**
     * 同时设置一个或多个 key-JSON.toJSONString(value) 对，当且仅当所有给定 key 都不存在
     *
     * @param maps
     * @return 之前已经存在返回false, 不存在返回true
     */
    ...
    public Integer append(String key, Object value) {
        return stringRedisTemplate.opsForValue().append(key, JSON.toJSONString(value));
    }
    ...
    public Boolean hPutIfAbsent(String key, String hashKey, Object value) {
        return stringRedisTemplate.opsForHash().putIfAbsent(key, hashKey, JSON.toJSONString(value));
    }
    ...
    public Long lLeftPushAll(String key, Object... value) {
        List<String> values = Arrays.stream(value).map(JSON::toJSONString).collect(Collectors.toList());
    ...
    public Long lLeftPushIfPresent(String key, Object value) {
        return stringRedisTemplate.opsForList().leftPushIfPresent(key, JSON.toJSONString(value));
    }
    ...
    public Long lLeftPush(String key, String pivot, Object value) {
        return stringRedisTemplate.opsForList().leftPush(key, pivot, JSON.toJSONString(value));
    }
    ...
    public Long lRightPush(String key, Object value) {
        return stringRedisTemplate.opsForList().rightPush(key, JSON.toJSONString(value));
    }
    ...
    public Long lRightPushIfPresent(String key, Object value) {
        return stringRedisTemplate.opsForList().rightPushIfPresent(key, JSON.toJSONString(value));
    }
    ...
    public Long lRightPush(String key, String pivot, Object value) {
        return stringRedisTemplate.opsForList().rightPush(key, pivot, JSON.toJSONString(value));
    }
    ...
    public Long lRemove(String key, long index, Object value) {
        return stringRedisTemplate.opsForList().remove(key, index, JSON.toJSONString(value));
    }
    ...
    public Boolean sMove(String key, Object value, String destKey) {
        return stringRedisTemplate.opsForSet().move(key, JSON.toJSONString(value), destKey);
    }
    ...
    public Double zIncrementScore(String key, Object value, double delta) {
        return stringRedisTemplate.opsForZSet().incrementScore(key, JSON.toJSONString(value), delta);
    }
    ...
    public String tryLock(String name, long expire) {
        return tryLock(name, expire, TimeUnit.MILLISECONDS);
    }
    ...
    public String tryLock(String key, long expire, TimeUnit timeUnit) {
        ...
        Boolean flag = stringRedisTemplate.opsForValue().setIfAbsent(key, token, expire, timeUnit);
